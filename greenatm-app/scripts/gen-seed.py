...
# ข้อที่ให้ขั้นที่ 2 "กำลังดำเนินอยู่" — เริ่มก่อนวันอ้างอิงแต่ยังไม่ถึงวันจบ
# มีไว้เพื่อให้สถานะ At Risk เกิดขึ้นได้จริง ไม่ใช่สถานะที่มีในตำนาน
IN_FLIGHT = {"1.4", "2.4"}

SLIP_REASON = "รอผลการพิจารณางบประมาณ"

# ชื่อขั้นงาน 3 ขั้นต่อรายการ ให้สอดคล้องเนื้องานจริงของข้อนั้น
# **จำนวนขั้นและวันแผนคงเดิมทุกข้อ** จึงไม่กระทบตัวเลขที่ verify:seed ยืนยันไว้
# (ถ้าเปลี่ยนจำนวนขั้น progressPercent ของ 4.1 จะไม่ใช่ 67% และ milestone เลยกำหนดจะไม่ใช่ 2)
MILESTONE_NAMES = {
  "1.1": ["ทบทวนนโยบายสิ่งแวดล้อมฉบับปัจจุบัน", "เสนอคณะกรรมการพิจารณา", "ประกาศใช้และสื่อสารทั่วองค์กร"],
  "1.2": ["จัดทำคู่มือระบบจัดการสิ่งแวดล้อม", "อบรมผู้ปฏิบัติงานตามคู่มือ", "ตรวจติดตามภายในรอบแรก"],
  "1.3": ["สำรวจความตระหนักของบุคลากร", "จัดกิจกรรมสร้างวัฒนธรรมสิ่งแวดล้อม", "วัดผลและสรุปบทเรียน"],
  "1.4": ["กำหนดตัวชี้วัดและค่าฐาน", "ตั้งเป้าหมายรายปีร่วมกับหน่วยงาน", "ติดตามผลไตรมาสแรก"],
  "2.1": ["สำรวจห้วงอากาศที่ปรับใช้ร่วมได้", "ประสานหน่วยงานที่เกี่ยวข้อง", "เริ่มใช้ FUA และเก็บผล"],
  "2.2": ["ประเมินความต้องการข้อมูลอุตุนิยมวิทยา", "เชื่อมต่อข้อมูลเข้าระบบปฏิบัติการ", "ทดสอบใช้งานจริงและปรับแก้"],
  "2.3": ["ประเมินพื้นที่อับสัญญาณเฝ้าตรวจ", "ติดตั้งและปรับปรุงอุปกรณ์", "ตรวจรับและวัดผลความครอบคลุม"],
  "2.4": ["ตั้งคณะทำงานร่วมกับท่าอากาศยาน", "ออกแบบการแลกเปลี่ยนข้อมูล A-CDM", "ทดลองใช้และประเมินผล"],
  "2.5": ["สำรวจจุดติดขัดบนภาคพื้น", "ปรับวิธีปฏิบัติการจัดการภาคพื้น", "วัดเวลาแท็กซี่ก่อน-หลัง"],
  "2.6": ["ออกแบบวิธีปฏิบัติการไต่ต่อเนื่อง", "ประสานสายการบินและซ้อมใช้", "เก็บผลการใช้จริง"],
  "2.7": ["ออกแบบวิธีปฏิบัติการร่อนต่อเนื่อง", "ตรวจวัดผลการใช้งานจริง", "ลงนามรับรองผลการตรวจวัด"],
  "2.8": ["ทบทวนเส้นทางบิน PBN ที่ใช้อยู่", "ออกแบบและประกาศเส้นทางใหม่", "ประเมินผลด้านเชื้อเพลิง"],
  "2.9": ["ศึกษาเกณฑ์ระยะห่างตามความปั่นป่วน", "ปรับเกณฑ์และอบรมเจ้าหน้าที่", "ติดตามความจุที่เปลี่ยนไป"],
  "2.10": ["รวบรวมข้อมูลวิถีบินที่ใช้จริง", "วิเคราะห์หาวิถีที่ประหยัดกว่า", "นำเสนอและเริ่มใช้"],
  "2.11": ["ทบทวนกระบวนการ ATFM", "ปรับปรุงเครื่องมือและขั้นตอน", "วัดผลความคล่องตัวรายเดือน"],
  "2.12": ["กำหนดหัวข้อวิจัยด้านสิ่งแวดล้อม", "ดำเนินการวิจัยและเก็บข้อมูล", "เผยแพร่ผลและนำไปใช้"],
  "3.1": ["ศึกษาความเหมาะสมการจัดการพลังงาน", "จัดหางบประมาณและดำเนินการ", "ติดตั้งและทดสอบระบบตรวจวัด"],
  "3.2": ["ศึกษาความเหมาะสมพื้นที่ติดตั้ง", "จัดหางบประมาณและผู้รับจ้าง", "ติดตั้งและทดสอบการจ่ายไฟ"],
  "3.3": ["ประเมินประสิทธิภาพอุปกรณ์ CNS", "เปลี่ยนอุปกรณ์ที่กินพลังงานสูง", "วัดผลการใช้พลังงานหลังเปลี่ยน"],
  "3.4": ["สำรวจปริมาณของเสียและการใช้น้ำ", "วางระบบคัดแยกและบำบัด", "ตรวจวัดคุณภาพน้ำทิ้ง"],
  "4.1": ["ทบทวนเกณฑ์จัดซื้อที่เป็นมิตรต่อสิ่งแวดล้อม", "ประกาศใช้กับการจัดซื้อรอบใหม่", "สรุปสัดส่วนการจัดซื้อสีเขียว"],
  "4.2": ["สำรวจประเด็นห่วงใยของชุมชน", "จัดเวทีรับฟังความเห็น", "รายงานผลและแผนตอบสนอง"],
  "4.3": ["แจ้งผู้มีส่วนได้ส่วนเสียเรื่องการปรับห้วงอากาศ", "รับฟังและปรับแผนตามความเห็น", "ประกาศการเปลี่ยนแปลงอย่างเป็นทางการ"],
  "4.4": ["สำรวจการเดินทางของบุคลากร", "จัดทางเลือกเดินทางที่ปล่อยคาร์บอนต่ำ", "วัดผลการเปลี่ยนพฤติกรรม"],
}
...
def milestones(code):
    ...
    if code in IN_FLIGHT:
        # ขั้นที่ 2 เริ่มแล้วแต่ยังไม่ถึงวันจบ -> ตามปฏิทินควรคืบหน้าไปแล้วราวครึ่งขั้น
        plan[1] = ("2026-07-01", "2026-11-20")
    out = []
    for i, (ps, pe) in enumerate(plan, start=1):
        done = 100 if i == 1 else (PCT[code] if i == 2 else 0)
        m = {
            "seq": i,
            "name": MILESTONE_NAMES.get(code, [f"ขั้นที่ {i}"] * 3)[i - 1],
            "weight": round(100 / len(plan), 2),
